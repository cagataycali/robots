...
| `STRANDS_MESH_ACCEPT_PERMISSIVE_ACL` | Acknowledgement token for a blacklist-shaped operator ACL. An `STRANDS_MESH_ACL_FILE` with `default_permission: "allow"` **and** one or more `rules` is refused at load with `PermissiveACLError` unless this variable is set to `1`/`true`/`yes` — the allow + rules shape is a blacklist policy where any gap in the rule set exposes the mesh, and the refusal exists so it is not shipped by copy-paste from a lab template. Does not silence the separate warning that fires when the built-in permissive default (allow + empty rules) is in use; that warning is silenced by supplying `STRANDS_MESH_ACL_FILE` with a `deny`-shaped policy, not by this token. Do not set on a production fleet — the acknowledgement does not make the ACL safer, it only records that an operator has accepted a blacklist posture | unset (refuses blacklist ACL) |
