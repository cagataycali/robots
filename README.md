...
| 2 | Policy post-tuning | `train_policy` (LeRobot / Cosmos 3 trainers) |
...
- **Any policy.** VLA models (LeRobot ACT/Pi0/SmolVLA/Diffusion, NVIDIA Cosmos 3),
...
graph LR
    ...
    C --> D[Policy Provider<br/>Cosmos 3 / LeRobot / planner / mock]
    D --> E[Action Chunk]
...
| `train_policy` | Post-tune (fine-tune) a policy on a recorded dataset (LeRobot / Cosmos 3 trainers, full or LoRA) |
...
name is the same one used for inference, so `create_policy("cosmos3")` and
`create_trainer("cosmos3")` refer to one provider. It returns a `Trainer` you
...
packages one of the local trainer paths (the `cosmos3` provider is
directly containerizable). Install with `pip install
...
<details>
<summary><b>Mesh / IoT env vars (advanced)</b></summary>
...
└── tools/                 # run_policy, lerobot_*, pose, serial, robot_mesh
```
