...
| `STRANDS_MESH_DISABLE_CA_PIN` | Set to `true` / `1` / `yes` to skip CA pin verification on the *download* path only. The on-disk re-use path always raw-checks the pin regardless. Last-resort break-glass; prefer `STRANDS_MESH_CA_PINS` for rotations. | `false` |
