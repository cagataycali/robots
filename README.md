...
| Valid cert, CN does not match any ACL rule | **Mitigated** when `STRANDS_MESH_ACL_FILE` is set with literal CN allowlists. **Not mitigated by default** — the built-in permissive ACL (`any_authenticated_peer`) admits any CA-signed peer to every key-expression. The default-state safety net is `STRANDS_MESH_ACCEPT_PERMISSIVE_ACL`: without that opt-in, `Mesh.start()` refuses to acquire a Zenoh session under the permissive default in `mtls` mode, so an operator who forgot to ship an ACL file fails closed instead of running wide-open. Production deployments must ship a literal-CN ACL file. |
