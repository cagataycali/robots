...
| `STRANDS_MESH_ACL_FILE` | Path to a JSON5 Zenoh ACL file. When unset the built-in permissive default applies (any CA-signed peer may publish/subscribe on any key); see `examples/mesh_acl_example.json5` (role-scoped) and `examples/mesh_acl_strict_per_peer.json5` (per-peer isolation). | unset |
| `STRANDS_MESH_POLICY_HOST_ALLOW` | Comma-separated allowlist of VLA policy-server hosts/CIDRs a robot may connect to for inference. The default permits loopback only (`localhost`, `127.0.0.1`, `::1`). | loopback only |
| `STRANDS_MESH_HITL_ACTIONS` | Which `robot_mesh` actions require a human-in-the-loop interrupt before they dispatch. `all` = every gateable action (incl. `subscribe`/`watch`); `none` = disable the gate (logs a warning); or a comma-separated subset of `emergency_stop,broadcast,tell,send,stop,subscribe,watch`. Unset = the physical-actuation default (`emergency_stop,broadcast,tell,send,stop`). | actuation default |
| `STRANDS_MESH_SUBSCRIBE_ALLOW` | Comma-separated extra Zenoh key-expr patterns the `robot_mesh` `subscribe` action may target, in addition to the built-in low-impact set (`**/presence`, `**/health`, `**/safety/**`). Prevents an agent subscribing to another peer's control/sensor streams. | shared classes only |
| `STRANDS_GR00T_IMAGE` | Container image the `gr00t_inference` tool runs. Must pass the image allowlist. The agent cannot choose the image. | `gr00t:latest` |
| `STRANDS_GR00T_IMAGE_ALLOW` | Comma-separated extra image-name patterns (trailing `*` = tag wildcard) added to the built-in allowlist (`gr00t:*`, `nvcr.io/nvidia/isaac-gr00t:*`). | built-in only |
