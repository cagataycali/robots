...
def test_teleoperate_status_keeps_telemetry_in_json_block(tmp_path, monkeypatch):
    ...
    # ``strands_robots.tools.lerobot_teleoperate`` names the decorated tool on the
    # package, so the module itself is resolved by name.
    ...
    monkeypatch.setattr(_session, "SESSION_DIR", tmp_path)
...
def test_train_status_keeps_telemetry_in_json_block(tmp_path, monkeypatch):
    ...
    # ``strands_robots.tools.lerobot_train`` names the decorated tool on the
    # package, so the module itself is resolved by name.
