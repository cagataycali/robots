...
These pin that the three fleets read differently, that the grading is not
re-implemented, and that the route carries both fields to the operator.
...
# ---------------------------------------------------------------- the route
def test_the_route_forwards_the_accounting_and_still_withholds_raw_replies():
    """The sheet reads signed_rail; the accounting has to survive the hop."""
    ...
    bridge.peers = {"arm-1": {"peer_id": "arm-1"}}
    bridge.live_peers = mock.Mock(return_value=["arm-1"])

    async def fake_send(peer, cmd, timeout=5.0, source="api"):
        return {"type": "response", "result": {"ok": True, "status": "stopped"}}

    bridge.send_cmd_async = fake_send
    bridge.signed_estop = mock.Mock(return_value=signed_estop_over([ACK, NO_STOP_TASK]))

    body = TestClient(app).post("/api/safety/estop").json()

    assert body["signed_rail"]["responses_received"] == 2
    assert body["signed_rail"]["peers_not_stopped"] == ["arm-2"]
    assert body["lockout_engaged"] is True
    assert "responses" not in body["signed_rail"], "raw per-peer replies stay out of the summary rail"
