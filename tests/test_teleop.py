...
# --- Q183: stop_teleop is host-agnostic (lives on the mixin) ---------------


class _StoppableSession:
    """Stands in for an InputReceiver/InputPublisher: stop() returns stats."""
    ...
    def stop(self):
        self.stopped += 1
        return {"source": self._source, "frames_received": 7, "hz_actual": 14.0}


def test_stop_teleop_exists_on_mixin_for_sim_shaped_hosts():
    """Q183: a host WITHOUT hardware's own stop_teleop (i.e. a sim) can still
    stop a receive it started — mesh teleop_stop dispatches via hasattr, so
    the method must live on TeleopMixin, not only on hardware Robot."""
    host = FakeHost()  # plain mixin host, no hardware override
    assert type(host).stop_teleop is TeleopMixin.stop_teleop
    rcv = _StoppableSession("leader-peer/leader")
    host._input_receivers = {"leader-peer/leader": rcv}

    res = host.stop_teleop()

    assert res["status"] == "success"
    assert rcv.stopped == 1
    assert host._input_receivers == {}
    assert "7 frames" in res["content"][0]["text"]


def test_stop_teleop_idle_is_safe_and_named_device_matches_suffix():
    host = FakeHost()
    assert host.stop_teleop()["content"][0]["text"] == "No active teleop sessions."

    keep = _StoppableSession("a/other")
    gone = _StoppableSession("b/leader")
    host._input_receivers = {"a/other": keep, "b/leader": gone}
    host.stop_teleop("leader")
    assert gone.stopped == 1 and keep.stopped == 0
    assert list(host._input_receivers) == ["a/other"]


def test_hardware_robot_stop_teleop_resolves_to_mixin():
    """The hardware duplicate is deleted; one implementation, no drift."""
    from strands_robots.hardware_robot import Robot

    assert Robot.stop_teleop is TeleopMixin.stop_teleop
