...
class TestBridgeSnapshotCarriesIt:
    """The badge must come from snapshot(), or the API and the websocket disagree.

    U2's role annotation was proved on /api/fleet and still did not render, because the
    UI draws from the ws snapshot. Same hook, same lesson.
    """

    def _bridge(self):
        from strands_robots.dashboard.mesh_bridge import MeshBridge

        b = MeshBridge.__new__(MeshBridge)  # no mesh, no threads
        import threading

        from strands_robots.dashboard import safety_state as ss

        b.peers = {}
        b._peers_lock = threading.RLock()
        b._coalesce_lock = threading.RLock()
        b._lockout = ss.Lockout()
        b._lockout_proof = {}
        b.activity = []
        b.peer_id = "dash"
        b._protected_peer_ids = lambda: set()
    ...
    def test_every_peer_in_the_snapshot_has_a_lockout_field(self) -> None:
        b = self._bridge()
        import time as _t

        b.peers["so101-arm-2"] = {"peer_id": "so101-arm-2", "last_seen": _t.time(), "first_seen": 1.0}
        snap = b.snapshot()
        peer = snap["peers"]["so101-arm-2"]
        assert "lockout" in peer, "an absent lockout field reads as 'fine' - that IS the bug"
        assert peer["lockout"]["state"] == "unknown"

    def test_a_seen_estop_marks_the_peer_locked_in_the_snapshot(self) -> None:
        b = self._bridge()
        import time as _t

        now = _t.time()
        b.peers["so101-arm-2"] = {"peer_id": "so101-arm-2", "last_seen": now, "first_seen": now - 100}
        from strands_robots.dashboard import safety_state as ss

        b._lockout = ss.apply_event(ss.Lockout(), kind="estop", data={"source": "evac-coordinator"}, now=now - 10)
        peer = b.snapshot()["peers"]["so101-arm-2"]
        assert peer["lockout"]["state"] == "locked"
        assert peer["lockout"]["by"] == "evac-coordinator"
        # and the rest of the peer is untouched
        assert peer["peer_id"] == "so101-arm-2"
