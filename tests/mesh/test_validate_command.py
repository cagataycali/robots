...
class TestExecuteStart:
    ...
    def test_execute_default_policy_host_is_localhost(self):
        out = sec.validate_command({"action": "execute", "instruction": "go"})
    ...
    def test_execute_policy_port_bounds(self):
        out = sec.validate_command({"action": "execute", "instruction": "go", "policy_port": 8080})
        assert out["policy_port"] == 8080

        with pytest.raises(sec.ValidationError, match="policy_port"):
            sec.validate_command({"action": "execute", "instruction": "go", "policy_port": 70000})
...
class TestSafeHostAndModel:
    ...
    def test_safe_server_address_rejects_ipv6_brackets(self):
        # Bracketed IPv6 literals are rejected (default allowlist is loopback;
        # operators wanting IPv6 should use a hostname).
        assert sec.is_safe_server_address("tcp://[::1]:5555") is False
