...
class TestF15WireCommandKeyRequired:
    ...
    and verify the dispatch is rejected (no robot-side execution
    occurs).
    """

    def test_flat_envelope_rejected_in_exec_cmd(self):
        """Drive a flat-shape envelope through ``_exec_cmd`` directly
        (bypassing the thread-spawn) and verify the wire-boundary
        check rejects it before reaching ``_dispatch``."""
        from strands_robots.mesh import core as mesh_core

        m = mesh_core.Mesh.__new__(mesh_core.Mesh)
        m.peer_id = "test-peer"

        # Track whether _dispatch is called -- it must not be.
        dispatched_args = []
        m._dispatch = lambda cmd: dispatched_args.append(cmd) or {"ok": True}
        m.publish = lambda key, data: None  # capture response publishes

        # Flat envelope: action/instruction/policy_provider at top level,
        # no `command` wrapper.
        flat = {
            "sender_id": "operator-1",
            "turn_id": "abc",
        ...
        assert dispatched_args == [], (
            f"F15-C: flat-shape envelope must be rejected before _dispatch; got dispatched_args={dispatched_args}"
        )
    ...
    def test_well_formed_envelope_with_command_key_dispatches(self):
        ...
        well_formed = {
            ...
            "turn_id": "abc",
            "command": {
                "action": "status",  # status is permitted under any lockout posture
            },
        }
        m._exec_cmd(well_formed)
        assert len(dispatched_args) == 1
        assert dispatched_args[0]["action"] == "status"
