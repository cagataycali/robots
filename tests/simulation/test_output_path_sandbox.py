...
class TestTheSymlinkRefusalLeavesSomewhereToGo:
    """Refusing a symlink is right; dead-ending the caller is not.

    On macOS the most reachable scratch directory on the machine IS a symlink -
    ``/tmp`` points at ``/private/tmp`` - so a caller asking for the most
    ordinary path available got a refusal that reads like an attack was
    detected, with no next move. The link is still never followed; the message
    now hands over the destination so the caller can pass it deliberately.
    """

    def test_the_refusal_names_the_destination_to_pass_instead(self, tmp_path):
        ...
        with pytest.raises(ValueError) as excinfo:
            validate_output_path(str(link), label="output_dir", sandbox_root=None, allow_abs=True)

        text = str(excinfo.value)
        assert "refusing to follow" in text, "the refusal itself must not soften"
        assert str(real) in text, "the caller needs the path that works"
        assert "pass that path instead" in text

    def test_it_still_refuses_rather_than_resolving_for_the_caller(self, tmp_path):
        """The hint is advice, not a silent redirect: following is the vector."""
        real = tmp_path / "real2"
        real.mkdir()
        link = tmp_path / "link2"
        link.symlink_to(real)
    ...
    def test_a_broken_link_promises_nothing(self, tmp_path):
        """A dangling link has no honest destination to offer."""
        ...
        text = str(excinfo.value)
        assert "refusing to follow" in text
        assert "pass that path instead" not in text or "does_not_exist" in text
