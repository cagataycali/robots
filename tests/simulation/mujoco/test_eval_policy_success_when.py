...
(missing body refused up front; a base clause on a fixed-base arm refused naming
the robot under evaluation), evaluated per step (a clause that never holds
...
def test_success_when_base_clause_on_a_fixed_base_arm_is_refused_naming_it(sim):
