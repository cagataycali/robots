...
- **Rate limits bound LLM-driven nuisance** independently of approval: `emergency_stop` at 3/min, `broadcast` at 10/min, `tell`/`send` at 30/min. A declined approval does not consume a slot, so declining nuisance prompts can never lock an operator out of a genuine emergency stop. A slot is reserved atomically at the point the action is known to run, so concurrent invocations cannot exceed the cap.
...
`validate_command` bounds every field of an incoming `execute` / `start` command before it reaches the dispatcher: the action must be in the allowlist, `duration` and `policy_port` are range-checked, `policy_host` / `server_address` / `model_path` are allowlist-gated, and every string field is length-bounded and refuses C0/DEL/C1 control characters. Natural-language fields bound only the control range - a non-ASCII instruction is admitted - while identifier fields such as `robot_name` stay printable-ASCII-only.
...
**Do not use this variable to work around a registry omission.** Adding a provider to `registry/policies.json` must include the corresponding edit to `_REGISTRY_POLICY_PROVIDERS` in `mesh/security.py`, which a guard test enforces. This variable extends the vocabulary *beyond* what the registry knows.
