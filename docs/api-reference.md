...
| Symbol | What |
|--------|------|
| `Simulation` | The `mujoco` backend's concrete class. |
...
| `TaskStatus` | Enum: idle / running / stopped / completed / failed. |
| `RobotTaskState` | Dataclass with current task info. |
...
| Symbol | What |
|--------|------|
| `Policy` | ABC — `get_actions`, `set_robot_state_keys`, `requires_images`. |
| `MockPolicy` | Sinusoidal-trace mock. |
| `create_policy(provider, **kw)` | Resolve provider + construct. |
| `register_policy(name, loader, aliases)` | Runtime registration. |
| `list_providers()` | All known provider names. |
| `UntrustedRemoteCodeError` | Raised when `STRANDS_TRUST_REMOTE_CODE` is required but not set. |
| `Gr00tPolicy` | NVIDIA GR00T client. |
| `LerobotLocalPolicy` | HuggingFace LeRobot inference. |
...
| Symbol | What |
|--------|------|
| `DatasetRecorder.create(repo_id, fps, robot_features, action_features, task)` | Factory. |
