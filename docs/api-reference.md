...
| `stop_policy(robot_name)` | Cooperatively stop a rollout, then wait (bounded, 2 s) for its worker to exit so the next call on that robot is not refused; the `json` block reports `was_running` and `worker_exited` (`false` = still winding down, said in a second sentence; `null` = nothing was running). An empty `robot_name` is refused naming the rollouts in flight. Refused (naming the class) by a backend with no durable per-robot claim. |
