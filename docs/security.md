...
- **`STRANDS_MESH_HITL_ACTIONS` tunes the gate.** You can widen it to `all` (which also gates the read-only `subscribe` / `watch` telemetry actions and the sim-only `sim_call`), narrow it to a comma-separated subset, or set it to `none`. Setting `none` re-opens the entire physical-actuation surface to the LLM without confirmation - the SDK logs a one-time warning when this is in effect. Do not use `none` outside a fully trusted, non-networked test.
