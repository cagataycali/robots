...
- **The `Robot` agent tool asks an operator before it drives real hardware.** `Robot("so100", mode="real")` handed to an `Agent` is the first path to metal in the README, and its motion actions - `execute` and `start` (a policy rollout) and the direct `set_joint_positions`, `set_gripper` and `set_torque enabled=true` - stop for an operator interrupt *before* anything is dispatched or written, through the same gate as `use_ros`; a declined or headless call dispatches nothing, and an agent that has not answered yet is paused rather than moved. A direct move is planned first (the arm is read, unknown joints and travel over the per-call cap - 20° or the config's `max_relative_target` - are refused) so the operator is shown the exact travel that will be written, joint by joint; a refusal costs no approval. Pre-approve by action name with `STRANDS_ROBOT_COMMAND_ALLOW=execute,start` (or `set_joint_positions`, or `*`) or bypass with `BYPASS_TOOL_CONSENT=true`. `set_torque enabled=false` releases the arm and is never gated: stopping is never harder than moving. `status` and `stop` are never gated, and neither are the observe actions `get_state`/`get_robot_state`, `list_cameras` and `render` - they read the servos and cameras and write nothing, and they exist so an agent asked where the arm is has a read to reach for instead of a rollout (before they existed, the agent's only route to a joint position was `execute` with a mock policy). The simulation tool never is - it touches no hardware. A grant the dashboard's motion hook deposited for the same call is spent instead of asking twice.
