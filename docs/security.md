...
`_acl_config._validate_acl_shape` refuses the second shape at ACL load with a `PermissiveACLError` unless `STRANDS_MESH_ACCEPT_PERMISSIVE_ACL` is set to `1`, `true`, or `yes` (case-insensitive, whitespace-stripped). The refusal exists so a copy-pasted lab ACL — where `default_permission: "allow"` + one or two convenience `rules` is common — cannot ship to a production fleet without an operator writing the acknowledgement variable, which forces the trade-off into a config-file review rather than a silent widening at deploy time.

- The built-in permissive default (`default_permission: "allow"` **with no rules**) is a *different* posture and does *not* reach this gate — it stays gated by `Mesh.start`'s separate refuse-to-start path when paired with `STRANDS_MESH_ACL_FILE` unset, and by a one-shot warning per session open when an operator ACL is loaded on top of it. `STRANDS_MESH_ACCEPT_PERMISSIVE_ACL` does not silence that warning; supply an ACL with `default_permission: "deny"` (see [`examples/mesh/mesh_acl_strict_per_peer.json5`](https://github.com/strands-labs/robots/blob/main/examples/mesh/mesh_acl_strict_per_peer.json5)) to silence it.
...
Reference: `strands_robots.mesh._acl_config._validate_acl_shape`, `strands_robots.mesh._acl_config.PermissiveACLError`.
