...
| `DEVICE_CONNECT_CLIENT_ID` | anonymous | Read as the caller id only when `STRANDS_ROBOT_MESH_AGENT_ID` is unset, so an allowlist entry can be satisfied by either variable. |
