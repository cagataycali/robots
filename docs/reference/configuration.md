...
| `MICRODUCK_MEDIA_SOCKET` | Unix socket `MicroduckDriver` reaches `mediad` on for camera frames. An empty or whitespace value falls back to the default | `/run/mediad/media.sock` |
| `MICRODUCK_TOF_SOCKET` | Unix socket the same driver reaches `tofd` on for time-of-flight range data. An empty or whitespace value falls back to the default | `/run/tofd/tof.sock` |
...
| `STRANDS_ROBOTS_NO_DYLD_SHIM` | Set `1` to disable the macOS auto-fix that puts Homebrew ffmpeg on the dyld path for torchcodec video streaming (see [Recording](../recording.md)). The shim marks its single re-exec with `_STRANDS_ROBOTS_DYLD_REEXEC`, which it sets itself so the re-exec cannot repeat; no operator sets it | unset |
...
| `STRANDS_MESH_NAMESPACE` | Fleet namespace prefix on every mesh key-expression. The Zenoh `namespace` config field provides routing isolation: two fleets with different namespaces exchange no messages even when their key-expressions collide. Must match on every peer of one fleet; a mismatch is silent (peers connect at the transport layer and exchange no application traffic). Empty / whitespace values fall back to the default so `STRANDS_MESH_NAMESPACE=""` cannot produce keys like `"//presence"` | `strands` |
...
| `STRANDS_MESH_ACCEPT_PERMISSIVE_ACL` | Acknowledgement token with **three** effects, all widening the mesh posture and all taken together: a blacklist-shaped `STRANDS_MESH_ACL_FILE` (`default_permission: "allow"` **and** `rules`) loads instead of raising `PermissiveACLError`, `Mesh.start` comes up under a permissive-by-shape ACL instead of refusing, and the per-session `WARNING` about the built-in permissive default is suppressed. Set to `1`/`true`/`yes` (case-insensitive, whitespace-stripped); `on` and every other spelling is not an acknowledgement. `strands-robots doctor` reads the same predicate, so its Mesh row agrees with the gates. See [Blacklist ACL acknowledgement](../security/mesh.md#blacklist-acl-acknowledgement-strands_mesh_accept_permissive_acl) | unset (refuses blacklist ACL, refuses to start under built-in permissive default, warns on every session open) |
| `STRANDS_MESH_POLICY_HOST_ALLOW` | Comma-separated allowlist of VLA policy-server hosts/CIDRs for inference | loopback only |
| `STRANDS_MESH_POLICY_TYPE_ALLOW` | Comma-separated extras appended to the built-in `policy_type` / `policy_provider` allowlist `validate_command` gates the mesh `execute` / `start` payloads against. The two vocabularies share one allowlist, so an entry widens both. Entries are compared lowercase and charset-validated against `^[a-z][a-z0-9_]*$`; a malformed entry drops with a WARNING naming the variable and the offender rather than widening the allowlist silently. The adjacent `policy_host`, `server_address`, `pretrained_name_or_path` and `model_path` gates are unaffected. See [command allowlists](../security/commands.md) | unset (built-in list only) |
...
| `SO101_RECORD_CONVERGE` | RTX render ticks the SO-101 arm is held still on a recorded pose, so the temporal upscaler settles before the frame is kept. Integer; raise on a slower GPU | `6` |
| `SO101_IDLE_CONVERGE` | The same tick count for an idle live-preview pose | `4` |
| `SO101_IDLE_RENDER_PERIOD` | Minimum seconds between idle live-preview refreshes. A static scene re-rendered at full speed pegs the RTX renderer and starves the recorder and HTTP threads | `1.0` |
