...
| `move_to: ... did not reach ... The servo was stopped: the robot is in contact: 'ground' <-> 'so100/Fixed_Jaw/geom_18' (d=-0.0002 m)` (or `commanded joint(s) at a limit: '...' at its upper limit`) | IK solved the point, but the servo could not get there: something on the robot is pushing against another geom, or a commanded joint sits on a bound of its range. The refusal names the pair (nearest first, at most three, total in `json.obstruction.contacts_total`) and the joint, read at the final tick. An unnamed geom is labelled `<body>/geom_<id>`, exactly as `get_contacts` labels it | Move the target away from what the robot touches, or pick one the named joint can reach inside its range. If the text says `no contact involved the robot and no commanded joint was at a limit`, nothing visible blocks it: raise `max_steps` or loosen `tol` - the arm may not have arrived yet, and a longer run is what reveals a contact that is still ahead of it. `get_contacts` shows the full list |
