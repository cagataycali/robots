...
| `get_robot_state` | `name` → joint positions, velocities, torques; an `end_effector` line naming the frame `move_to` drives, its world position, its offset **from the base** and the horizontal axis the arm currently extends along (`the arm currently extends along -Y` for an SO-100/SO-101 at home) - so "in front of the robot" resolves to the same side of the base for the agent and the person. The `json` payload carries `end_effector.base`, `from_base` and `extends_along` (`"+X"`/`"-Y"`/…, `null` when the arm is over its base) |
