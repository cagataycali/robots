...
The rollout's three **posture** flags are held to a domain of their own for a different reason. `fast_mode`, `reset_between` and `wbc_install_torque_control` each select a branch rather than scaling a quantity, so there is nothing to clamp and no partial effect, and each was read by truthiness - which inverts the two spellings a caller reaches for. Measured on a MuJoCo rollout, every row returning `status="success"`: `fast_mode="false"` ran 30 steps at 30Hz in 0.006s instead of the 1.001s `False` takes, issuing the whole rollout as one burst rather than paced on a deadline; `reset_between="no"` reset between episodes while `0` / `""` / `None` / `[]` did not, so every episode after the first began in the state the previous one ended in and was still recorded as independent; and `wbc_install_torque_control=None` withheld the torque shim a position-servo humanoid needs for a stable gait. All three are now checked on `boolean_flag_error`, the domain the recording postures and the mesh wire schema already share - the wire schema in particular refuses this exact `fast_mode` field unless it is a `bool` before forwarding it to the same rollout, so a value an untrusted `tell()` could not get past the transport was reachable from every local caller. The check is a structured error at the public entry points (`run_policy`, `start_policy`, and the `run_policy` tool, which checks it before its own recording step can replace a dataset at `dataset_root`) and a `ValueError` on `PolicyRunner.run`, which is drivable directly. `reset_between` is checked only when `n_episodes > 1`, the condition under which it is read at all, exactly as `duration` and `max_steps` are checked only when they set the horizon.
