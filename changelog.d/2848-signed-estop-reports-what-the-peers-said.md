...
The sheet's sentence is derived from those two fields rather than from the latch,
and leads with a peer that reported not stopping when there is one: a lockout
refuses the NEXT command and does not halt motion already underway, which is the
distinction that decides whether an operator walks to the power supply. A peer
that never replied is named as uncounted rather than folded into either total.
