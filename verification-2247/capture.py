...
main_src = subprocess.run(
    ["git", "show", "upstream/main:scripts/check_merge_base_overlap.py"],
    cwd=ROOT, capture_output=True, text=True).stdout
assert main_src.strip(), "could not read the checker from upstream/main"
assert main_src != branch_src, "the two arms must differ"
...
facts["mutations"] = [
    [
        ...
        2,
        [
            "test_a_renamed_path_overlaps_a_sibling_editing_the_old_name",
            "test_the_two_modes_agree_about_a_rename"
        ]
    ],
    [
        ...
        1,
        [
            "test_a_head_side_at_the_compare_cap_stays_in_the_pairwise_comparison"
        ]
    ],
    [
        ...
        2,
        [
            "test_a_head_side_path_set_spanning_several_pages_is_read_whole",
            "test_a_truncated_head_side_path_set_is_named_rather_than_read_as_no_overlap"
        ]
    ]
]
facts["verification"] = {
    "head": "939cd2382c3b",
    "base": "7954c914d3c8",
    "suite": "29588 passed, 266 skipped, 0 failed",
    "elapsed_s": 696.45,
    "pristine": 29584,
    "cases_in_file": 47,
    "lint": "ruff check / ruff format / mypy clean; scripts/ ruff + mypy clean",
    "live_open": 8,
    "live_pairs": 28,
    "live_blocking": "#1035 + #1722",
    "live_stale": 1
}
save()
