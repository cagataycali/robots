...
reproduced = sum(1 for _n, a, _names in M if a > 0)
assert reproduced == len(M) == 3, (reproduced, len(M))
assert sum(len(names) for _n, _a, names in M) == 5
...
put(axm, 0.5, 1.03, "3a. #2247's mutation table, independently reproduced on Thor",
    ha="center", fontsize=12.0, fontweight="bold", transform=axm.transAxes)
put(axm, 0.045, 0.905, "mutation applied to #2247's head", fontsize=10.2, fontweight="bold",
    color=GREY, transform=axm.transAxes)
put(axm, 0.650, 0.905, "pins that fail", fontsize=10.2, fontweight="bold", color=GREY,
    transform=axm.transAxes)
TOPM, LASTM = 0.800, 0.400
SM = (TOPM - LASTM) / (len(M) - 1)
assert SM > 0.030, SM
y = TOPM
for label, n_fail, names in M:
    put(axm, 0.045, y, label[:50], fontsize=9.6, color="#24292f", transform=axm.transAxes, **MONO)
    put(axm, 0.650, y, str(n_fail), fontsize=9.6, fontweight="bold", color=RED,
        transform=axm.transAxes, **MONO)
    for k, nm in enumerate(names):
        put(axm, 0.685, y - 0.048 * k, nm[:44], fontsize=8.2, color=GREY,
            transform=axm.transAxes, **MONO)
    y -= SM
assert abs((y + SM) - LASTM) < 1e-9
put(axm, 0.045, 0.265,
    "3 of 3 rows reproduce, test name for test name, on #2247's own head.",
    fontsize=10.2, fontweight="bold", transform=axm.transAxes)
VER = F["verification"]
put(axm, 0.045, 0.175,
    f"head {VER['head']} / base {VER['base']}:  {VER['suite']}  in {VER['elapsed_s']:.0f}s",
    fontsize=9.6, color=GREY, transform=axm.transAxes, **MONO)
put(axm, 0.045, 0.110,
    f"  = pristine {VER['pristine']} + the {VER['cases_in_file'] - 43} new cases.  {VER['lint']}.",
    fontsize=9.6, color=GREY, transform=axm.transAxes, **MONO)
put(axm, 0.045, 0.045,
    f"  live sweep on that head: {VER['live_open']} open / {VER['live_pairs']} pairs, "
    f"blocking {VER['live_blocking']}, {VER['live_stale']} stale base.",
    fontsize=9.6, color=GREY, transform=axm.transAxes, **MONO)
