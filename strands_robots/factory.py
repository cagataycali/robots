...
_UNIFIED_ROBOTS: Dict[str, Dict[str, Any]] = {
    "so100": {
        "sim": "so100",
        "real": "so100_follower",
        "description": "TrossenRobotics SO-ARM100 (6-DOF)",
    },
    "so101": {
        "sim": "so101",
        "real": "so101_follower",
        "description": "RobotStudio SO-101 (6-DOF)",
    },
    "koch": {
        "sim": "koch",
        "real": "koch_follower",
        "description": "Koch v1.1 Low Cost Robot Arm (6-DOF)",
    },
    "unitree_g1": {
        "sim": "unitree_g1",
        "real": "unitree_g1",
    ...
    "aloha": {
        "sim": "aloha",
        "real": "bi_so_follower",
        "description": "ALOHA Bimanual (2× ViperX 300s)",
    },
    "unitree_h1": {
        "sim": "unitree_h1",
        "real": None,
    ...
    "stretch3": {
        "sim": "stretch3",
        "real": None,
        "description": "Hello Robot Stretch 3",
    },
    "openarm": {
        "sim": "openarm",
        "real": None,
    ...
    "leap_hand": {
        "sim": "leap_hand",
        "real": None,
        "description": "LEAP Hand (16-DOF)",
    },
    "robotiq_2f85": {
        "sim": "robotiq_2f85",
        "real": None,
        "description": "Robotiq 2F-85 Gripper",
    },
    "fourier_n1": {
        "sim": "fourier_n1",
        "real": None,
        "description": "Fourier N1 Humanoid (26-DOF)",
    },
    "apollo": {
        "sim": "apollo",
        "real": None,
        "description": "Apptronik Apollo Humanoid",
    },
    ...
    "asimov_v0": {
        "sim": "asimov_v0",
        "real": None,
        "description": "Asimov V0 Bipedal Legs (12-DOF)",
    },
    "earthrover": {
        "sim": None,
        "real": "earthrover",
...
_ALIASES: Dict[str, str] = {
    ...
    "agilex_piper": "piper",
    "unitree_g1_wbc": "unitree_g1",
    "g1_wbc": "unitree_g1",
    "earthrover_mini_plus": "earthrover",
    "earth_rover": "earthrover",
    "frodobots": "earthrover",
    "omx_follower": "omx",
    "omx_robot": "omx",
    "robotis_omx": "omx",
    "bi_openarm_follower": "bi_openarm",
    "dual_openarm": "bi_openarm",
    "openarm_bimanual": "bi_openarm",
}
    ...
    if robot_info and robot_info.get("real"):
        try:
            ...
            robot_ports = [
                ...
                and not any(
                    s in p.description.lower()
                    for s in ["bluetooth", "internal", "debug", "apple", "modem"]
                )
            ]
            if robot_ports:
                logger.info(
                    f"Auto-detected robot hardware: {[p.device for p in robot_ports]}"
                )
                return "real"
    ...
    if mode == "sim":
        if backend == "isaac":
            ...
            if result.get("status") == "error":
                raise RuntimeError(
                    f"Failed to create Isaac robot '{canonical}': {result}"
                )
            ...
            if result.get("status") == "error":
                raise RuntimeError(
                    f"Failed to create Newton robot '{canonical}': {result.get('message', result)}"
                )
            ...
            if result.get("status") == "error":
                raise RuntimeError(
                    f"Failed to create sim robot '{canonical}': {result}"
                )
            return sim
