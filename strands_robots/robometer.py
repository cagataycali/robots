...
def _setup_batch_collator(
    processor: Any, tokenizer: Any, config: Any, *, is_eval: bool = True
) -> Any:
...
class StrandsRobometerRewardWrapper:
    ...
    def __init__(
        ...
        self._success_votes: collections.deque = collections.deque(
            maxlen=self._success_vote_window
        )
    ...
    def _ensure_model_loaded(self) -> None:
        if self._model_loaded:
            return
        self._config, self._tokenizer, self._processor, self._model = (
            _load_robometer_model(self._model_path, device=self._device)
        )
    ...
    def step(
        ...
        reward = (
            (env_reward + scaled_reward)
            if self._add_estimated_reward
            else scaled_reward
        )
        ...
        info["robometer_progress"] = (
            self._prev_progress if self._prev_progress is not None else 0.0
        )
...
def robometer_reward_fn(
    ...
    def _ensure_loaded() -> None:
        if loaded[0]:
            return
        config, tokenizer, processor, model = _load_robometer_model(
        ...
        artefacts["collator"] = _setup_batch_collator(
            processor, tokenizer, config, is_eval=True
        )
        loaded[0] = True
...
class RobometerTrainer:
    ...
    def train(
        self, train_dataset: Any = None, eval_dataset: Any = None, **kwargs: Any
    ) -> Dict[str, Any]:
...
def strands_robots_loader(
    ...
    * A path (``str`` / ``pathlib.Path``) to a LeRobot v3 dataset.
            ...
            episodes.append(
                {
                    ...
                    or getattr(data_source, "task", None),
                }
            )
    elif isinstance(data_source, (str, pathlib.Path)):
        episodes = _load_lerobot_v3_episodes(
            pathlib.Path(data_source), pixels_key=pixels_key
        )
    else:
        raise TypeError(
    ...
    for ep in episodes:
        ...
        if len(frames) > max_frames:
            ...
            if actions_list is not None:
                actions_list = [
                    actions_list[min(i, len(actions_list) - 1)] for i in indices
                ]
    ...
    logger.info(
        "Loaded %d trajectories from %s", len(trajectories), type(data_source).__name__
    )
    return trajectories
...
class RobometerBenchmark:
    ...
    def _rollout_policy(
        ...
        for _ in range(self._num_episodes):
            ...
            trajectories.append(
                {"frames": frames, "task_instruction": self._task_instruction}
            )
        return trajectories
    ...
    def evaluate(
        ...
        for name, policy in policies.items():
            logger.info(
                "Evaluating policy %r (%d episodes) …", name, self._num_episodes
            )
            env = env_factory()
            try:
        ...
        ranking = sorted(
            policy_scores, key=lambda n: policy_scores[n]["mean_progress"], reverse=True
        )
