...
class Go1Policy(Policy):
    ...
    def _ensure_server_loaded(self):
        ...
        try:
            ...
            logger.info(
                f"🤖 GO-1 server connected: {self._server_url} (status={resp.status_code})"
            )
    ...
    def _ensure_local_loaded(self):
        ...
        try:
            ...
            logger.info(
                f"🤖 GO-1 loaded: {n_params / 1e9:.1f}B params in {elapsed:.1f}s on {device}"
            )
            ...
            for name in ["state", "action"]:
                self._data_stats[name] = {
                    "mean": torch.from_numpy(
                        np.array(stats_json[name]["mean"], dtype=np.float32)
                    ),
                    "std": torch.from_numpy(
                        np.array(stats_json[name]["std"], dtype=np.float32)
                    ),
                }
    ...
    def _build_server_payload(
        ...
        for go1_key, image in cameras.items():
            ...
            if image is not None:
                payload[server_key] = (
                    image.tolist() if isinstance(image, np.ndarray) else image
                )
    ...
    def _infer_local(
        ...
        if self._data_stats and "state" in self._data_stats:
            state_tensor = self._normalize_tensor(
                state_tensor, self._data_stats["state"]
            )
