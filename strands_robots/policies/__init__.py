        ...
        try:
            ...
            for attr_name in dir(module):
                ...
                if (
                    ...
                    logger.info(
                        f"Auto-discovered policy provider: {name} -> {attr_name}"
                    )
...
_registry.register(
    ...
    loader=lambda: importlib.import_module(
        "strands_robots.policies.lerobot_async"
    ).LerobotAsyncPolicy,
)
...
_registry.register(
    "dreamgen",
    loader=lambda: importlib.import_module(
        "strands_robots.policies.dreamgen"
    ).DreamgenPolicy,
)
...
_registry.register(
    "dreamzero",
    loader=lambda: importlib.import_module(
        "strands_robots.policies.dreamzero"
    ).DreamzeroPolicy,
...
def create_policy(provider: str, **kwargs) -> Policy:
    ...
    .. warning:: Security — trust_remote_code
        ...
        Most HuggingFace VLA providers load models with ``trust_remote_code=True``,
        which executes arbitrary Python from the model repository. Only load models
        from organisations you trust. See each provider's ``__init__.py`` for details.
