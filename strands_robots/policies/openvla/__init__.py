...
class OpenvlaPolicy(Policy):
    ...
    def _ensure_loaded(self):
        ...
        attn_impl = (
            "flash_attention_2"
            if self._use_flash_attn and device.startswith("cuda")
            else None
        )
        kwargs = {
        ...
        self._vla = AutoModelForVision2Seq.from_pretrained(self._model_id, **kwargs).to(
            device
        )
        ...
        logger.info(
            f"🔮 OpenVLA loaded: {n_params/1e9:.1f}B params in {elapsed:.1f}s on {device}"
        )
        self._loaded = True
