...
class InternvlaPolicy(Policy):
    ...
    def _ensure_loaded(self):
        ...
        try:
            ...
            logger.info(
                f"🧠 InternVLA loaded: {n_params/1e9:.1f}B in {elapsed:.1f}s on {device}"
            )
        except Exception as e:
            raise ImportError(
    ...
    async def _infer_server(
        ...
        actions = np.array(
            result.get("actions", result.get("action", [0.0] * self._action_dim))
        )
    ...
    def _infer_local(
        ...
        with torch.no_grad():
            if hasattr(self._model, "predict_action"):
                ...
                action_text = self._processor.decode(
    ...
    def _parse_action_text(self, text: str) -> np.ndarray:
        ...
        numbers = re.findall(
            r"[-+]?\d*\.?\d+", text.split("Out:")[-1] if "Out:" in text else text
        )
    ...
    def _array_to_dict(self, arr: np.ndarray) -> Dict[str, Any]:
        ...
        keys = (
            ...
            if len(self._robot_state_keys) >= len(arr)
            else eef_keys
        )
