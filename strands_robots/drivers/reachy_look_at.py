...
def _coordinates_error(u: Any, v: Any, width: Any, height: Any) -> str | None:
    for name, value in (("u", u), ("v", v), ("frame_width", width), ("frame_height", height)):
        if isinstance(value, bool) or not isinstance(value, int):
            return f"plan_look_at: {name} must be an integer"
    if width <= 0 or height <= 0 or width * height > 16_000_000:
        return "plan_look_at: frame dimensions must be positive and at most 16 megapixels"
    if not (0 <= u < width and 0 <= v < height):
        return "plan_look_at: pixel is outside the supplied frame dimensions"
    return None
...
def _numeric(value: Any, shape: tuple[int, ...], name: str) -> Any:
    ...
    if raw.shape != shape or any(isinstance(x, bool) or not isinstance(x, numbers.Real) for x in raw.flat):
        raise ValueError(f"plan_look_at: {name} must be a numeric array of shape {shape}")
    result = raw.astype(float)
    if not np.isfinite(result).all():
        raise ValueError(f"plan_look_at: {name} must contain only finite values")
    return result
...
def _pixel_plan(specs: dict[str, Any], pose: dict[str, Any], u: int, v: int, width: int, height: int) -> dict[str, Any]:
    ...
    if specs.get("name") not in ("wireless", "lite", "older_rpi"):
        raise ValueError("plan_look_at: unknown camera calibration size; supported models are wireless/lite/older_rpi")
    rows = specs.get("available_resolutions")
    if not isinstance(rows, list):
        raise ValueError("plan_look_at: camera did not return available resolutions")
    crops = []
    for row in rows:
        if isinstance(row, dict) and row.get("width") == width and row.get("height") == height:
            ...
            if isinstance(crop, bool) or not isinstance(crop, int | float) or not np.isfinite(crop) or crop <= 0:
                raise ValueError("plan_look_at: invalid crop factor")
            crops.append(float(crop))
    if len(set(crops)) != 1:
        raise ValueError("plan_look_at: frame resolution is unsupported or its crop factor is ambiguous")
    crop = crops[0]
    k = _numeric(specs.get("K"), (3, 3), "K")
    if k[0, 0] <= 0 or k[1, 1] <= 0 or not np.allclose(k[2], [0, 0, 1]) or k[0, 1] != 0 or k[1, 0] != 0:
        raise ValueError("plan_look_at: K is not a supported pinhole intrinsic matrix")
    d = specs.get("D")
    if not isinstance(d, list) or len(d) not in (4, 5, 8, 12):
        raise ValueError("plan_look_at: D must have 4, 5, 8 or 12 distortion coefficients")
    ...
    if not np.allclose(head[3], [0, 0, 0, 1], atol=1e-7) or rotation_error > 0.01 or np.linalg.det(rot) <= 0:
        raise ValueError("plan_look_at: head pose is not a near-rigid right-handed transform")
    ...
    if not np.isfinite(xy).all() or not np.isfinite(error_px) or error_px > 0.1:
        raise ValueError("plan_look_at: lens inversion did not converge within 0.1 pixel; no pinhole fallback")
