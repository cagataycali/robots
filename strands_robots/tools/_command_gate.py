...
instruction, or an arm motion named by the pose action carrying it. Such a caller
names its own allowlist variable and its own way of matching it; the interrupt,
the
...
def gate_motion(
    ...
    *,
    ...
    :func:`gate_command`), :mod:`~strands_robots.tools.serial_tool` and
    :mod:`~strands_robots.tools.pose_tool`, so the
