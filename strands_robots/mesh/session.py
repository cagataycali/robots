...
def _apply_fallback_topology(cfg: Any, local_ep: str, scheme: str) -> str:
    ...
    child-to-dashboard topic works, because that is the link the child opened
...
def _get_zenoh_session_directly() -> Any | None:
    ...
    with _SESSION_LOCK:
        ...
        if not connect_env and not listen_env:
            ...
            # bridge-transport peer just went permanently dark. Both modes
            # here retry ``connect/endpoints`` in the background, so a
            # restarted hub re-links automatically (measured for client mode:
            # delivery resumed after a 6s hub outage with no process restart).
            cfg = _build_config()
            fallback_mode = _apply_fallback_topology(cfg, local_ep, scheme)
            try:
                _SESSION = zenoh.open(cfg)
