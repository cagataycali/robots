...
class SensorLoopsMixin:
    ...
    def _pose_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                if pose:
                    self._put_signed(f"strands/{self.peer_id}/pose", pose)
    ...
    def _health_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                if health:
                    self._put_signed(f"strands/{self.peer_id}/health", health)
            except Exception as exc:  # noqa: BLE001
    ...
    def _imu_loop(self) -> None:
        ...
        while self._running:
            try:
                imu = self._read_imu()
                if imu:
                    self._put_signed(f"strands/{self.peer_id}/imu", imu)
            except Exception as exc:  # noqa: BLE001
    ...
    def _odom_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                if odom:
                    self._put_signed(f"strands/{self.peer_id}/odom", odom)
            except Exception as exc:  # noqa: BLE001
    ...
    def _lidar_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                if summary:
                    self._put_signed(f"strands/{self.peer_id}/lidar/summary", summary)

                if now - last_state_publish >= state_period:
                    state = self._read_lidar_state()
                    if state:
                        self._put_signed(f"strands/{self.peer_id}/lidar/state", state)
            ...
            try:
                ...
                if hands:
                    for hand_name, hand_data in hands.items():
                        self._put_signed(f"strands/{self.peer_id}/hand/{hand_name}/state", hand_data)
            except Exception as exc:  # noqa: BLE001
    ...
    def _map_info_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                if info:
                    self._put_signed(f"strands/{self.peer_id}/map/info", info)
            except Exception as exc:  # noqa: BLE001
    ...
    def publish_safety_event(
        ...
        self._put_signed(f"strands/{self.peer_id}/safety/event", event)
