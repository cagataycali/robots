...
(``$STRANDS_BASE_DIR/user_robots.json`` - read through :mod:`._overlay`, the
leaf this module and :mod:`.user_registry` share so neither imports the other;
see :func:`_merge_user_robots`).  The
...
def _validate_robots(data: dict) -> None:
    ...
    """
    # Imported lazily: the driver seam reads the registry, so importing it at
    # module scope would close an import cycle across packages. A driver name is validated here rather than where it
