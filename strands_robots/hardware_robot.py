...
class Robot(TeleopMixin, AgentTool):
    ...
    def _policy_requires_error(
        ...
        real-arm step did exactly this (lab PC2-016).
        ...
        missing = [
            ...
            if key not in ("port", "host") and (policy_kwargs.get(key) is None or policy_kwargs.get(key) == "")
        ]
        if not missing:
            return None
        hints = {
            ...
            "content": [
                {
                    "text": (
                        ...
                        "Without it the task would start, energize the arm and fail at its first action."
                    )
                }
            ],
        }
