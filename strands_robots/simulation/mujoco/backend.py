"""MuJoCo lazy import, API floor and rendering probes.

GL backend *selection* - the ``MUJOCO_GL`` vocabulary, the headless detection
and :func:`~strands_robots._mujoco_gl._configure_gl_backend` - lives in
:mod:`strands_robots._mujoco_gl`, a stdlib-only leaf the package root can run
at import time without importing this package (#3587). The names re-imported
below are the ones the probes here still call.
"""
...
def _software_rendering_remedy() -> str:
    ...
    :func:`~strands_robots._mujoco_gl._ensure_nvidia_egl_vendor_icd` decides on.
