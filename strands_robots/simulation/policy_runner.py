...
class PolicyRunner:
    ...
    def run(
        ...
        *,
        ...
        step_count = 0
        # The ticker is acquired on an ExitStack rather than released by hand.
        # A plain ``with Ticker(...)`` cannot express this one: the pace is
        # conditional on ``fast_mode``, and the ticker is read inside a closure
        # spanning both step loops below, so its live region is this whole
        # ``try`` rather than a single loop body. ``enter_context`` is the
        # standard library's answer to both, and it puts the release where the
        # rest of the package's paced loops have it - in the language, not in a
        # discipline each loop has to remember. Every paced loop in the package
        # is held to that, so a bare construction here is a suite failure rather
        # than two leaked descriptors per rollout.
        ...
        with contextlib.ExitStack() as pacing:
            try:
                ...
                if not fast_mode:
                    ...
                    ticker = pacing.enter_context(_Ticker(1.0 / control_frequency))
