...
``server``, ``auth``, ``record_api`` and everything else in the package, and the
refusal names ``strands-robots[dashboard]`` -- the extra that actually supplies
the missing module. ``require_optionals`` (plural) is deliberate: the absent
extra means several modules are missing at once, and reporting them one
ImportError at a time turns one install into three round trips.
